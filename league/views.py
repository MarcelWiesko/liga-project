...
def api_best_player(request):
    ...
    for player in players:
        goals_count = Goal.objects.filter(player=player).count()
        ranking.append({
            "player_id": player.id,
            "player": str(player),
    ...
    ranking = sorted(ranking, key=lambda x: x["goals"], reverse=True)

    if not ranking:
        return Response({"message": "Brak zawodników w bazie"})
...
def api_best_player_against_team(request, team_id):
    ...
    ).exclude(player__team=selected_team)
    ...
    if not ranking_list:
        return Response({
            "message": "Brak goli przeciwko tej drużynie"
        })

    return Response(ranking_list[0])